    ...
    for octave, octave_base in enumerate(octaves[::-1]):  # itterating backward
        ...
        if w > maxWidth:
            ...
            for i1 in range(0, 2):
                for j1 in range(0, 2):
                    spl_detail = detail[
                        :, i1 * h / 2 : (i1 + 1) * h / 2, j1 * w / 2 : (j1 + 1) * w / 2
                    ]
                    spl_octave_base = octave_base[
                        :, i1 * h / 2 : (i1 + 1) * h / 2, j1 * w / 2 : (j1 + 1) * w / 2
                    ]
...
def main(
    ...
    if stepsize is None:
        stepsize = 1.5
    if blendflow is None:
        blendflow = 0.75
    if blendstatic is None:
        blendstatic = 0.1
    if layers is None:
        ...
        for v in range(len(vids)):
            if var_counter < len(vids):
                ...
                if blendflow == 0:
                    ...
                    halludiff = (
                        morphPicture(preframe, saveframe, blendflow, preview)
                        - previousImg
                    )
                ...
                hallu_flow_masked = cv2.bitwise_and(
                    hallu_flow, hallu_flow, mask=flow_mask
                )

                ## blending the are with no optical flow
                background_blendimg = cv2.addWeighted(
                    img, (1 - blendstatic), hallu, blendstatic, 0
                )
...
def extractVideo(inputdir, outputdir):
    print(
        subprocess.Popen(
            "ffmpeg -i " + inputdir + " -f image2 " + outputdir + "/image-%06d.png",
            shell=True,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t",
        "--modelspath",
        help="Path to the directory containing the caffe models.",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "-m",
        "--modelname",
        help="Name of the caffe model to use.",
        type=str,
        required=False,
    ...
    parser.add_argument(
        "-bm",
        "--blendflow",
        help="Blend Amount for flow area. Default: 0.5",
        type=float,
        required=False,
    )
    parser.add_argument(
        "-bs",
        "--blendstatic",
        help="Blend Amount for static area. Default: 0.5",
        type=float,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-e", "--extract", help="Extract Frames From Video.", type=int, required=False
    )
    parser.add_argument(
        "-c", "--create", help="Create Video From Frames.", type=int, required=False
    )
    parser.add_argument(
        "-g", "--guide", help="Guided dream image input.", type=str, required=False
    )
    parser.add_argument(
        "-flow", "--flow", help="Optical Flow.", type=int, required=False
    )
    parser.add_argument(
        "-flowthresh",
        "--flowthresh",
        help="Optical Flow Masking threshhold. [0-255]",
        type=int,
        required=False,
    )
    parser.add_argument(
        "-gpu", "--gpu", help="Use GPU or CPU.", type=int, required=False
    )
    parser.add_argument(
        "-f", "--framerate", help="Video creation Framerate.", type=int, required=False
    )

    parser.add_argument(
        "-d",
        "--divide",
        help="Dividing into smaller piece: 0:none, 1:maxWidth/maxHeight, 2:half",
        type=int,
        required=False,
    )
    parser.add_argument(
        "-mw", "--maxWidth", help="maxiumu width to split.", type=int, required=False
    )
    parser.add_argument(
        "-mh", "--maxHeight", help="maxiumu height to split.", type=int, required=False
    )
